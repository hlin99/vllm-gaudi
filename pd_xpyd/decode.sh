...
bash disaggregated_prefill_server_launcher.sh -n 8 -d 16 --node-ip $1 --node-size 2 --node-rank $3 --dp-master-ip $2 --max-model-len 16384 --max-num-batched-tokens 16384 --max-num-seqs 32 -r decode --async --nixl-buffer-device hpu --log-dir /workspace --inc /workspace/vllm-gaudi/pd_xpyd/inc_ep16/maxabs_quant_g2_ep16.json $4
